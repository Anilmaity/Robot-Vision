...
def process_video(vid_file_path, frame_list):
    ...
    cap = cv2.VideoCapture(vid_file_path)  # capturing the video
    for frame_number in frame_list:  # loop to run algorithm for each frame
        # setting the current frame same as frame number
        cap.set(1, frame_number)
        _, frame = cap.read()
        # print(frame)
        # converting to hsv format
        hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
        # defining hsv lower range for red color
        lower_range = np.array([0, 200, 175])
        # defining hsv upper range for red color
        upper_range = np.array([8, 255, 255])
        # masking the frame accordingly
        mask = cv2.inRange(hsv, lower_range, upper_range)
        kernel = np.ones((5, 5), np.uint8)  # defing a 5X5 pixel area
        # removing any 5X5 pixel area in white to avoid detection
        mask = cv2.erode(mask, kernel)
        contours, image = cv2.findContours(
            mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  # creating contour around circle
        temp_list = []
        for cnt in contours:  # loop for the contours in each frame
            area = cv2.contourArea(cnt)
            approx = cv2.approxPolyDP(cnt, 0.01*cv2.arcLength(cnt, True), True)
            # drawing contours of approximated contours
            cv2.drawContours(frame, [approx], 0, (0), 5)
            # print(len(approx))
            if len(approx) >= 11 and len(approx) <= 14:  # verifying for circles
                M = cv2.moments(cnt)
        ...
        frame_details[frame_number] = temp_list

...
def applyPerspectiveTransform(input_img):
    ...
    input_img = cv2.resize(input_img, dimension, interpolation=cv2.INTER_AREA)
    ...
    contour_img = cv2.drawContours(input_img, contours, -1, (255, 0, 0), 4)
   ...
   # end ponits of maze
    pts2 = np.float32([[0, 0], [2000, 0], [0, 2000], [2000, 2000]]) # end point of image which is 500*500
    M = cv2.getPerspectiveTransform(pts1, pts2)
    warped_img = cv2.warpPerspective(contour_img, M, (2000, 2000))  # warping image

    # some times the end points are not aligned with the image so manually assign black colour to the border
    warped_img[0::, 0:4] = 0
    warped_img[0:4, 0::] = 0
    warped_img[0::, height-4:height] = 0
    warped_img[width-4:width, 0::] = 0

    ##################################################

    cv2.imshow("resized", warped_img)
    cv2.waitKey()
